...
define('IMAP_SERVER', getenv('ZPUSH_IMAP_SERVER') ?: 'localhost');

// connecting to default port (143)
define('IMAP_PORT', getenv('ZPUSH_IMAP_PORT') ? filter_var(getenv('ZPUSH_IMAP_PORT'), FILTER_VALIDATE_INT) : 143);

// best cross-platform compatibility (see http://php.net/imap_open for options)
define('IMAP_OPTIONS', getenv('ZPUSH_IMAP_OPTIONS') ?: '/notls/norsh');
...
define('IMAP_FOLDER_CONFIGURED', getenv('ZPUSH_IMAP_FOLDER_CONFIGURED') ? filter_var(getenv('ZPUSH_IMAP_FOLDER_CONFIGURED'), FILTER_VALIDATE_BOOLEAN) : false);
...
define('IMAP_EXCLUDED_FOLDERS', getenv('ZPUSH_IMAP_EXCLUDED_FOLDERS') ?: '');
...
define('IMAP_SMTP_METHOD', getenv('ZPUSH_IMAP_SMTP_METHOD') ?: 'mail');
...
if (getenv('ZPUSH_IMAP_SMTP_HOST')) {
        $imap_smtp_params['host'] = getenv('ZPUSH_IMAP_SMTP_HOST');
}
if (getenv('ZPUSH_IMAP_SMTP_PORT')) {
        $imap_smtp_params['port'] = filter_var(getenv('ZPUSH_IMAP_SMTP_PORT'), FILTER_VALIDATE_INT);
}
if (filter_var(getenv('ZPUSH_IMAP_SMTP_AUTH'), FILTER_VALIDATE_BOOLEAN)) {
        $imap_smtp_params['auth'] = true;
        if (getenv('ZPUSH_IMAP_SMTP_USERNAME')) {
                $imap_smtp_params['username'] = getenv('ZPUSH_IMAP_SMTP_USERNAME');
        }
        if (getenv('ZPUSH_IMAP_SMTP_PASSWORD')) {
                $imap_smtp_params['password'] = getenv('ZPUSH_IMAP_SMTP_PASSWORD');
        }
}
...
if (getenv('ZPUSH_IMAP_DISABLE_AUTHENTICATOR')) {
        define('IMAP_DISABLE_AUTHENTICATOR', getenv('ZPUSH_IMAP_DISABLE_AUTHENTICATOR'));
}
